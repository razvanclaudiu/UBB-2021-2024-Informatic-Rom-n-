...
void printM(Multime& m){
	...
	while (im.valid()) {
		cout << nr << " | ";
		nr++;
		std::cout << im.element() << " " << im.urm() << std::endl;
		im.urmator();
	}
...
void testDiferentaMinMax()
{
	...
	m.adauga(16);
	assert(m.diferentaMaxMin() == 11);
	m.adauga(30);
	assert(m.diferentaMaxMin() == 25);
	m.adauga(0);
	assert(m.diferentaMaxMin() == 30);
	m.adauga(10000);
	assert(m.diferentaMaxMin() == 10000);
	m.adauga(-10000);
	assert(m.diferentaMaxMin() == 20000);
	m.sterge(-10000);
	assert(m.diferentaMaxMin() == 10000);
}
...
void testAllExtins() {
	...
	testDiferentaMinMax();
	cout << "Test DiferentaMinMax Succes" << endl;
