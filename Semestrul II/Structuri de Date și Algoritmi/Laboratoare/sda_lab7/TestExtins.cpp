...
void testColectiaValorilor()
{
	...
	assert(d.colectiaValorilor().size() == 11);
	for (int i = 0; i <= 10; i++) {
		assert(d.colectiaValorilor().at(i) == i + 1);
	}
	for (int i = 10; i >= 0; i--) {
		d.sterge(i, i + 1);
	}
	assert(d.colectiaValorilor().size() == 0);
