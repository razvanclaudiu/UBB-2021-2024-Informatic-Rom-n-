...
public:
	...
	virtual void adauga_repo(Masina& m);
...
class FileMasinaRepository : public MasinaRepository
{
private:
	string filename;

	//Functia de generare a unei liste de masini luate dintr-un fisier
	//Post: se va genera o lista de masini in repository cu masinile din fisier
	//Throws: daca fisierul nu poate fi dechis atunci se va arunca o exceptie
	void loadFromFile();
	...
	void saveToFile();
public:
	FileMasinaRepository(string fname) :MasinaRepository(), filename{ fname } {
		loadFromFile();
	};

	//Functia de adaugare in repository si fisier
	//Parametrii: m - const Masina&, entitate care urmeaza a fi adaugata
	//Post: Se va adauga pe ultima pozitie a vectorului si pe ultima linie a fisierului o noua entitate Masina 
	//Throws: Daca exista o entitate Masina cu acelasi numar de inmatriculare deja 
	//	      existent sau fisierul nu poate fi deschis, se va arunca RepositoryException
	void adauga_repo(Masina& m) override;
};
...
public:
	...
	//Post: lista de masini va fi stocata in format CVS intr-un fisier
	//Throws: Daca fisierul nu s-a putut deschide atunci se va arunca o exceptie
	void lst_export_repo(string fname);
};
